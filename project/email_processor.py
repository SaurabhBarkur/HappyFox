...
def batch_modify_messages(message_ids, add_label_ids=None, remove_label_ids=None):
...
def fetch_email():
    ...
    for msg in messages:
        ...
        if value > 0:
            res_data = base64.urlsafe_b64decode(
                txt["payload"]["body"].get("data", "")
            ).decode("utf-8")
        else:
            res_data = ""
            for p in txt["payload"]["parts"]:
                data = base64.urlsafe_b64decode(p["body"].get("data", "")).decode(
                    "utf-8"
                )
